...
if uploads:
    ...
    for i, file in enumerate(uploads, start=1):
        ...
        with col2:
            link_inputs.append(st.text_input(
                f"ApprovalMax Link #{i}", key=f"link_{i}"))
        with col3:
            proof_inputs.append(st.text_input(
                f"Payment Proof #{i}", key=f"proof_{i}"))
...
st.download_button("⬇️ Download CSV", csv,
                   "requisition_output.csv", "text/csv")
